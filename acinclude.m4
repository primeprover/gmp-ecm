...
AC_ARG_ENABLE(gpu,
  AS_HELP_STRING([--enable-gpu=GPU_ARCH],
                 [Enable the cuda version, by default builds with all possible compute capabilities to build a single compute capability pass --enable-gpu=XX [default=no]]),
  [ AS_IF([test "x$enableval" = "xno"],
    [ enable_gpu="no" ],
    [ enable_gpu="yes"
      AS_CASE(["x$enableval"],
        [ xyes ], [],
        [ x[[2-9]][[0-9]] ], [ GPU_ARCH="$enableval" ],
...
AS_IF([test "x$enable_gpu" = "xyes" ],
  [
    ...
    VALID=""
    m4_foreach_w([compute_compatibility], [30 32 35 37 50 52 53 60 61 62 70 72 75],
      [
        testcc=compute_compatibility
        AS_IF([test -z "$GPU_ARCH" -o "$GPU_ARCH" = "$testcc"],
          [
